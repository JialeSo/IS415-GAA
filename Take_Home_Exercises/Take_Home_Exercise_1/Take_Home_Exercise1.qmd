...
observed_table <- data.frame(
  ...
  K_Obs = L_Yangon_CSR$obs[closest_indices],        # Observed K-function values
  K_Lo = L_Yangon_CSR$lo[closest_indices],          # Lower bound of CSR envelope
  K_Hi = L_Yangon_CSR$hi[closest_indices]           # Upper bound of CSR envelope
)
# Display the observed table
print(observed_table)
```
...
3.  **L-function Analysis**:
    ...
    1.  **Observed Tables**:
        ...
        |----------|----------|-----------|-----------|
        | 4.99309  | 24.08843 | 4.778285  | 5.207894  |
        | 9.98618  | 40.78681 | 9.771375  | 10.200984 |
        | 14.97927 | 50.83615 | 14.764465 | 15.194074 |
        | 20.02663 | 55.51990 | 19.811827 | 20.241437 |
        | 25.01972 | 58.32548 | 24.804917 | 25.234526 |
...
observed_table <- data.frame(
  ...
  K_Obs = L_Mandalay_CSR$obs[closest_indices],        # Observed K-function values
  K_Lo = L_Mandalay_CSR$lo[closest_indices],          # Lower bound of CSR envelope
  K_Hi = L_Mandalay_CSR$hi[closest_indices]           # Upper bound of CSR envelope
)
...
4.  **Observed Tables**:
    ...
    | **Distance** | **K_Obs** | **K_Lo**  | **K_Hi** |
    |--------------|-----------|-----------|----------|
    | 9.962643     | 48.22199  | 9.791656  | 10.13363 |
    | 20.016687    | 58.77544  | 19.845699 | 20.18767 |
    | 29.979331    | 71.02485  | 29.808343 | 30.15032 |
    | 40.033374    | 81.98111  | 39.862387 | 40.20436 |
...
## Yangon Quarter Year KDE Animation
...
## Mandalay Quarter Year KDE Animation
...
### 10.1.1 Yangon's Clark-Evans Test for Spatial Randomness:

```{r}
Yangon_ClarksEvan_Temporal <- clarkevans.test(Yangon_QuarterYear_PPP_Numeric, 
                              correction = "none", 
                              clipregion = Yangon_Owin, 
                              alternative = c("clustered"),
                              nsim = 30)
print(Yangon_ClarksEvan_Temporal)
```

10.1.2 Yangon's Spatial temporal Ripley K Function

```{r}

K_Yangon_ST <- Kest(Yangon_QuarterYear_PPP_Numeric, correction = "Ripley")

# Step 2: Plot the L-function, showing L(d) - r
plot(K_Yangon_ST, . - r ~ r, ylab = "K(d) - r", xlab = "d(km)", main = "Ripley' K-Function for Yangon Civilian Related Conflicts")
```

```{r}
# Step 2: Perform the spatio-temporal K-function
Yangon_K_ST <- Kest.stppp(Yangon_StPP)

# Step 3: Plot the spatio-temporal K-function
plot(Yangon_K_ST, main = "Spatio-Temporal K-Function for Yangon Conflicts")
